...
(begin (define x (if (integer? nil) 2 3)) x)
; expect 3
(begin (define x (if (integer? nil) 2 3)) '(x what))
; expect (x what)

((lambda (x y) (begin (define x (if (integer? x) 2 3)) (+ x y) )) 5 3)
; expect 5

(define (rats rats) (* rats rats))
(rats (begin (define (rats rats) (if (null? rats) what rats)) (rats 4)))
; expect 4

(and (or (and (or (and (and (and nil nil) 3) 1) nil) #f) #t 4) #t)
; expect True

(cond (nil) (nil) (nil) (else nil))
; expect ()

(cond ((rats (begin (define (rats rats) (if (null? rats) what rats)) (rats 4))) 'rats)
  (else errorwhatisthing))
; expect rats

(define yolo 'yolo)
(define swag 'swag)
(let ( (yolo 420) (swag 360) ) ((lambda (yolo swag) (+ yolo swag)) yolo swag))
; expect 780
...
(begin (display 7) (/ 35 7))
; expect 75

(lambda (x y) (* 4 5))
; expect (lambda (x y) (+ 4 5))

(lambda (k) (print k) (* 32 4))
; expect (lambda (k) (print k) (* 32 4))
