...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        callFrame = procedure.env.make_call_frame(procedure.formals, args)
        val = scheme_eval(procedure.body, callFrame)
        return val


    elif isinstance(procedure, MuProcedure):
        callFrame = env.make_call_frame(procedure.formals, args)
        val = scheme_eval(procedure.body, callFrame)
        return val
    else:
...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("Number of formals does not match the number of values")
...
def constructPair(lst):
    if lst is nil:
        return nil
    return Pair(lst.first, constructPair(lst.second))
#################
# Special forms #
#################
...
def do_lambda_form(vals, env):
    ...
    if len(vals) > 2:
        begin_expression = Pair("begin", constructPair(vals.second))
        return LambdaProcedure(formals, begin_expression, env)
...
def do_mu_form(vals):
    ...
    if len(vals) > 2:
        begin_expression = Pair("begin", constructPair(vals.second))
        return MuProcedure(formals, begin_expression)
    ...
    if scheme_symbolp(target):
        ...
        def check(x):
            if not scheme_symbolp(x):
                raise SchemeError("{0} is not a good name".format(x))
            return x
        target.map(check)
        formals = constructPair(target.second)
        
        body = constructPair(vals.second)
        f = do_lambda_form(Pair(formals, body), env)
        env.bindings[target[0]] = f
        return target[0]
    else:
...
def do_and_form(vals, env):
    ...
    output = True
    for val in vals:
        output = scheme_eval(val, env)
        if scheme_false(output):
            return False
    return output
...
def do_cond_form(vals, env):
    ...
    for i, clause in enumerate(vals):
        ...
        if scheme_true(test):
            if clause.second is nil:
                return scheme_eval(quote(clause[0]), env)
...
def check_formals(formals):
    ...
    for formal in formals:
        if not scheme_symbolp(formal):
            raise SchemeError("{0} is not a good formal name".format(formal))
        if formal not in lst:
            lst.append(formal)
        else:
            raise SchemeError("{0} exists already".format(formal))
