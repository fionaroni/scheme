...
(define (list-partitions total max-pieces max-value)
  ...
  (define (helper n m final p)
    (cond 
      ((eq? n 0) (cons final nil))
      ((< n 0) nil)
      ((eq? m 0) nil)
      ((eq? p 0) nil)
      (else 
        (append
          (helper (- n m) m (cons m final) (- p 1))
          (helper n (- m 1) final p)
        )
      )
    )
  )
  (helper total max-value nil max-pieces)
)
...
(define (add tree entry)
  (if (eq? (length tree) 0) nil
    (cons (+ entry (car tree)) 
          (add (cdr tree) entry)
    )
  )
...
(define (flat children)
  (if (eq? (length children) 0)
    nil
    (append (tree-sums (car children)) (flat (cdr children)))
  )
)


(define (tree-sums tree)
  (if (eq? (length (children tree)) 0)
    (cons (entry tree) nil)
    (add (flat (children tree))
         (entry tree)
    )
  )
)
